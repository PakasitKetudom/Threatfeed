    ...
    <div id="main">
             ...
             <select name="time"  style="width:100%;height:50px;text-align: center;"  onchange="this.form.submit();" >
                ...
                <?php
                    if($_REQUEST['time']==""){?>
                        ...
                        <option  value="<?php echo $_REQUEST['time']; ?>"><?php echo 'Select  :   '.$_REQUEST['time']; ?></option>
                     <?php } ?>
        ...
        <div class="chart-container" style="position: relative; height:20vh; width:60vh; margin:0 auto;">
            ...
            if($selected_time==$lastweek){?>
                ...
                <?php 
                //minus 1
                 $sqlminus1 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus1."%' ";
                 $resultminus1 = mysqli_query($conn,$sqlminus1);
                 while($row = mysqli_fetch_assoc($resultminus1)) {
                    $dateminus1 =  $row['Number of Rows'];   
                 }
                //minus 2
                 $sqlminus2 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus2."%' ";
                 $resultminus2 = mysqli_query($conn,$sqlminus2);
                 while($row = mysqli_fetch_assoc($resultminus2)) {
                    $dateminus2 =  $row['Number of Rows'];   
                 }
                 //minus 3
                 $sqlminus3 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus3."%' ";
                 $resultminus3 = mysqli_query($conn,$sqlminus3);
                 while($row = mysqli_fetch_assoc($resultminus3)) {
                    $dateminus3 =  $row['Number of Rows'];   
                 }
                 //minus 4
                 $sqlminus4 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus4."%' ";
                 $resultminus4 = mysqli_query($conn,$sqlminus4);
                 while($row = mysqli_fetch_assoc($resultminus4)) {
                    $dateminus4 =  $row['Number of Rows'];   
                 }
                 //minus 5
                 $sqlminus5 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus5."%' ";
                 $resultminus5 = mysqli_query($conn,$sqlminus5);
                 while($row = mysqli_fetch_assoc($resultminus5)) {
                    $dateminus5 =  $row['Number of Rows'];   
                 }
                 //minus 6
                 $sqlminus6 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus6."%' ";
                 $resultminus6 = mysqli_query($conn,$sqlminus6);
                 while($row = mysqli_fetch_assoc($resultminus6)) {
                    $dateminus6 =  $row['Number of Rows'];   
                 }
                 //minus 7
                 $sqlminus7 = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$minus7."%' ";
            ...
            <?php }elseif($selected_time==$lastmonths){ ?>
                ...
                <?php 
                //jan
                $sqljan = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstJanuary' AND '$lastJanuary' ";
                $resultmijan = mysqli_query($conn,$sqljan);
                while($row = mysqli_fetch_assoc($resultmijan)) {
                   $datejan =  $row['Number of Rows'];   
                }
                //feb
                $sqlfeb = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstFebruary' AND '$lastFebruary' ";
                $resultmifeb = mysqli_query($conn,$sqlfeb);
                while($row = mysqli_fetch_assoc($resultmifeb)) {
                   $datefeb =  $row['Number of Rows'];   
                }
                //March
                $sqlmar = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstMarch' AND '$lastMarch' ";
                $resultmimar = mysqli_query($conn,$sqlmar);
                while($row = mysqli_fetch_assoc($resultmimar)) {
                   $datemar =  $row['Number of Rows'];   
                }
                //April
                $sqlapr = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstApril' AND '$lastApril' ";
                $resultmiapr = mysqli_query($conn,$sqlapr);
                while($row = mysqli_fetch_assoc($resultmiapr)) {
                   $dateapr =  $row['Number of Rows'];   
                }
                //May
                $sqlmay = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstMay' AND '$lastMay' ";
                $resultmimay = mysqli_query($conn,$sqlmay);
                while($row = mysqli_fetch_assoc($resultmimay)) {
                   $datemay =  $row['Number of Rows'];   
                }
                //June
                $sqljune = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstJune' AND '$lastJune' ";
                $resultmijune = mysqli_query($conn,$sqljune);
                while($row = mysqli_fetch_assoc($resultmijune)) {
                   $datejune =  $row['Number of Rows'];   
                }
                //July
                $sqlju = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstJuly' AND '$lastJuly' ";
                $resultmiju = mysqli_query($conn,$sqlju);
                while($row = mysqli_fetch_assoc($resultmiju)) {
                   $dateju =  $row['Number of Rows'];   
                }
                //August
                $sqlaug = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstAugust' AND '$lastAugust' ";
                $resultmiaug = mysqli_query($conn,$sqlaug);
                while($row = mysqli_fetch_assoc($resultmiaug)) {
                   $dateaug =  $row['Number of Rows'];   
                }
                //September
                $sqlsep = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstSeptember' AND '$lastSeptember' ";
                $resultmisep = mysqli_query($conn,$sqlsep);
                while($row = mysqli_fetch_assoc($resultmisep)) {
                   $datesep =  $row['Number of Rows'];   
                }
                //October
                $sqloct = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstOctober' AND '$lastOctober' ";
                $resultmioct = mysqli_query($conn,$sqloct);
                while($row = mysqli_fetch_assoc($resultmioct)) {
                   $dateoct =  $row['Number of Rows'];   
                }
                //November
                $sqlnov = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstNovember' AND '$lastNovember' ";
                ...
                //December
                 $sqlde = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp between '$firstDecember' AND '$lastDecember' ";
            ...
            <?php }else{ ?>
                ...
                <?php $sql = "SELECT count(timestamp)as 'Number of Rows' from domainblocklist where timestamp LIKE '%".$today."%' ";
        ...
        <table class="pure-table pure-table-horizontal" cellpadding="8" cellspacing="1" border="1" width="100%">
                    <?php 
                     ...
                     $sqlc = "select * from domainblocklist LIMIT $start_from, ".$results_per_page; 
                    
                     
                    $result = $conn->query($sqlc);
                    if ($result->num_rows > 0) {?>
                    <tr>
    	                <th style="text-align:center; width:150px; background-color:#DCDCDC; color:#1E90FF" >Domain</th>
                        ...
                        <tr>
                        <td style="text-align:center;"><?php echo $row['domain'];?></td>
                ...
                <?php
                 $sqldownload = "select * from domainblocklist INTO OUTFILE '".$today.".txt'"; 
                ...
                <?php
                    $sqlcount = "select * from domainblocklist";
                ...
                //    //  echo ">".$i."</a> "; 
                //        echo "<a href='domainblock.php?time=".$today."&page=".$i."'>".$i."</a> ";
                //     }
               ?>
                <?php
                    ...
                    for( $p=1; $p<=$total_pages;$p++){
                        if( $p==$page){
                            ...
                            //echo "<a href='?page=$p&fname=$search'>$p</a>  &nbsp;";
                            //echo "<a href='searchcve.php?query=".$query."&sort=".$sort."&page=".$p."'></a>";
                            echo "<a href='domainblock.php?time=".$today."&page=".$p."'>$p</a>";
                            ?>
                            <?php
                        }//if
                    }//for
